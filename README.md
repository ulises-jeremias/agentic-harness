...
The `agent-toolkit workspace init` command scaffolds:

- `AGENTS.md` — AI orchestrator instructions
- `knowledge/` — persistent memory across sessions
- `loops/` — loop engineering templates
- `packs/` — context bundles per project/client
- `personas/` — focused work modes

### Using agent-toolkit CLI from this workspace

```bash
agent-toolkit workspace context   # session state snapshot
agent-toolkit memory inject       # inject all knowledge for context
agent-toolkit loop run daily-triage
agent-toolkit devcompanion queue my-project --request "..."
```
